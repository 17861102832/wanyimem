"""万忆中枢 v5 核心引擎（engine）
WanYiCore + 23 个 MCP 工具 + MCP_TOOLS 注册表 + JSON-RPC 请求分发。

本模块由 1.1.0 架构重组拆分而来：数据层见 core_base，stdio 传输层见 transport。
`wanyi.memory_core` 保留为兼容门面，旧导入路径全部可用。
"""
import hashlib
import json
import os
...
try:  # 包内导入（wanyi.engine）
    from .version import __version__
except ImportError:  # 顶层导入双模兼容
    from version import __version__
...
class WanYiCore:
    ...
    def tool_confidence_check(self,
        ...
        # 护栏模式（v1.0.6-patch）：full(默认，完整拦截) / warn(只提醒不拦截，极致交易模式) / off(完全关闭)
        # 通过环境变量 WANYI_GUARD_MODE 配置；warn 模式下 BLOCK 一律降级为 CAUTION，仅提醒不再拦截
        guard_mode = os.environ.get("WANYI_GUARD_MODE", "full").strip().lower()

        if risk["is_risky"] and current_conf < threshold:
            if guard_mode == "warn":
                verdict = "CAUTION"
                warnings.append("护栏模式=warn：原判定 BLOCK 已降级为提醒，不拦截你的交易决策")
            elif guard_mode == "off":
                verdict = "PASS"
            else:
                verdict = "BLOCK"
